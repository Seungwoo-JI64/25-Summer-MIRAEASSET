...
#### ① `기업 재무제표 및 건전성 보고서`
 &nbsp;**재무제표**에 기반하여 **기업의 건전성**을 파악하는 것은 투자를 하기 전 선행되어야 하는 주된 과제이다. 개인이 해당 과정을 진행하는 것은 상당히 번거로운 일이므로 미리 이를 생성한다면 큰 도움이 된다. 또한 `최종 분석 보고서`의 근거자료에도 이것이 활용되기 때문에 그 중요도는 높다. DART공시 API에서 제공하는 재무제표 데이터를 수집하였으며, KOSPI 100에 해당하는 기업들이 해당되며, 2023년~2025년 1분기 각 기업의 매출액, 당기순이익, 영업이익 등이 포함된다. 이를 기반으로 건전성 보고서를 생성한다. 여기엔 기업의 유동성, 수익성, 안전성의 분석 결과가 나타나며 투자 시 주의점과 리스크 요인도 보여준다. 건전성 보고서는 HyperCLOVA X를 통해 작성하였다. 재무제표 데이터와 그에 맞는 프롬프트를 작성하여 CLOVA X에 전달하였고, 그 예시는 아래와 같다. 부채 수준, 현금흐름, 자산 활용 등 다양한 요소를 종합적으로 고려하기 때문에 기업의 재무 상태가 튼튼하고 안전한지 알 수 있다.
